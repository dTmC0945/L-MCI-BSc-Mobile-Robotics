...
chown -R $USER:$USER $HOME
sed -i \
    "s/password = WebUtil.getConfigVar('password');/password = '$VNC_PASSWORD'/"\
...
# colcon and set ROS2 environmental variables
BASHRC_PATH=$HOME/.bashrc
grep -F "source /opt/ros/$ROS_DISTRO/setup.bash" \
     $BASHRC_PATH || echo "source /opt/ros/$ROS_DISTRO/setup.bash" >> $BASHRC_PATH
grep -F "source /usr/share/colcon_argcomplete/hook/colcon-argcomplete.bash" \
     $BASHRC_PATH || echo "source /usr/share/colcon_argcomplete/hook/colcon-argcomplete.bash" >> $BASHRC_PATH
...
echo "-------------------------------------------------------------------------"
echo "Booting up Linux + ROS2 Image. Hope you have fun :)"
echo "-------------------------------------------------------------------------"
